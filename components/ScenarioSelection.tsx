...
import { MessageSquare, Users, Wifi, Briefcase, Home, Heart, Zap, Loader2, RefreshCw } from 'lucide-react';
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="text-center mb-10">
        <h2 className="text-3xl font-serif text-slate-800 mb-4">Choose Your Practice Arena</h2>
        <p className="text-slate-500 max-w-lg mx-auto mb-6">
          The AI generates unique, high-stress scenarios for you to practice your Heart-Centered communication techniques.
        </p>
        
        {!loading && (
          <button 
            onClick={fetchScenarios}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-slate-200 shadow-sm text-slate-600 rounded-full text-sm font-medium hover:bg-slate-50 hover:text-sky-600 transition-colors"
          >
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-slate-400 animate-pulse">Designing conflict scenarios...</p>
        </div>
      ) : error ? (
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {scenarios.map((scenario) => (
            <button
              ...
              className="flex flex-col text-left bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md hover:border-sky-100 transition-all duration-300 group h-full"
            >
              ...
              <p className="text-sm text-slate-500 mb-4 line-clamp-3 flex-grow">{scenario.description}</p>
